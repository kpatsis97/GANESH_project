...
| `malnourished` | Whether the infant was classified as malnourished | Boolean | `True`, `False` |
...
| `Diagnosed_disorder` | Whether the infant received a formal neurodevelopmental disorder diagnosis | Binary | `1` = Diagnosed, `0` = No diagnosis; `NaN` = missing |

## Notes
- `NaN` indicates missing data throughout the dataset.
- `Aberrant_GM_MOS_below20` and `Aberrant_GM_MOS_below23` are derived from `Final_MOS_score` using two different clinical thresholds.
- The MOS (Movement Optimality Score) is a standardised assessment of general movement quality in infants, used as an early predictor of neurodevelopmental outcome.
